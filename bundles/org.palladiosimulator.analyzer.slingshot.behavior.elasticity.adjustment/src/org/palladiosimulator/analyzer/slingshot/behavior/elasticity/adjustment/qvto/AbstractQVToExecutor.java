package org.palladiosimulator.analyzer.slingshot.behavior.elasticity.adjustment.qvto;
...
import org.palladiosimulator.analyzer.slingshot.behavior.elasticity.adjustment.qvto.util.ModelTransformationCache;
import org.palladiosimulator.analyzer.slingshot.behavior.elasticity.adjustment.qvto.util.QVToModelCache;
...
public abstract class AbstractQVToExecutor {
    ...
    private static final Function<EObject, Collection<EObject>> CREATE_NON_EMPTY_MODEL_ELEMENTS_SWITCH = e -> Collections
        .singletonList(e);

    /**
     * Accumulates the model elements and finally creates a basic model extent.
     */
    private static final Collector<EObject, List<EObject>, ModelExtent> BASIC_MODEL_EXTENT_COLLECTOR = Collector
        .of(ArrayList<EObject>::new, (acc, t) -> acc.add(t), (l, r) -> {
            l.addAll(r);
            return l;
    ...
    public boolean executeTransformation(final URI transformationURI) {
        final Optional<QVToModelTransformation> data = this.transformationCache
    ...
    protected final ExecutionDiagnostic doExecution(final QVToModelTransformation modelTransformation,
            final ExecutionContext executionContext, final ModelExtent[] params) {
        return modelTransformation.getTransformationExecutor()
            .execute(executionContext, params);
    }
    ...
    protected boolean handleExecutionResult(final ExecutionDiagnostic executionResult) {
        ...
        LOGGER.log(level, String.format("%s; %s", executionResult.getMessage(), chainedDetails),
    ...
    protected ExecutionContext setupExecutionContext() {
        // setup the execution environment details -> configuration properties, LOGGER, monitor
        // object etc.
    ...
    protected ModelExtent[] setupModelExtents(final QVToModelTransformation transformation) {
        ...
        for (final TransformationParameterInformation inParams : transformation.getInParameters()) {
            ...
            if (sourceModel.isEmpty()) {
                throw new IllegalStateException("No model in QVTo model cache for parameter "
                        + (inParams.getParameterIndex() + 1) + " of type " + inParams.getParameterType()
                            .getName()
                        + " for transformation '" + transformation.getTransformationName() + "'");
            }
