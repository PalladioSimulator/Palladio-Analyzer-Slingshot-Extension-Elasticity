...
/**
 * The trigger checker for {@link QueueLength}. The measurements are only considered if the
 * measuring point references a passive resource (i.e. is of type {@link PassiveResourceReference}
 * and the referenced passive resource corresponds to the passive resource returned by
...
public class QueueLengthTriggerChecker extends TriggerChecker<QueueLength> {
    ...
    public FilterResult doProcess(final FilterObjectWrapper event) {
        if (event.getEventToFilter() instanceof final MeasurementMade measurementMade
                && isCorrectPassiveResource(measurementMade.getEntity()
                    .getMeasuringPoint())
    ...
    /**
     * Checks whether the measuring point is of type {@link PassiveResourceReference} and the
     * referenced passive resource matches with the passive resource by the {@link QueueLength}.
     * 
    ...
    private boolean isCorrectPassiveResource(final MeasuringPoint measuringPoint) {
        if (measuringPoint instanceof final PassiveResourceReference prmp) {
            return prmp.getPassiveResource()
                .getId()
                .equals(queueLength.getPassiveResource()
                    .getId());
        }
