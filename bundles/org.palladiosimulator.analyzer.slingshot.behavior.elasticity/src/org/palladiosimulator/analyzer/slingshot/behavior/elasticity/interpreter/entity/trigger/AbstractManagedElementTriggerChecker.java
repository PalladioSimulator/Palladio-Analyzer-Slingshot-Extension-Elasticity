...
import org.palladiosimulator.analyzer.slingshot.behavior.elasticity.interpreter.entity.aggregator.FixedLengthWindowSimpleAggregation;
import org.palladiosimulator.analyzer.slingshot.behavior.elasticity.interpreter.entity.aggregator.SlidingTimeWindowAggregationBasedOnEMA;
import org.palladiosimulator.analyzer.slingshot.behavior.elasticity.interpreter.entity.aggregator.WindowAggregation;
...
public abstract class AbstractManagedElementTriggerChecker<T extends ManagedElementsStateStimulus>
    ...
    public AbstractManagedElementTriggerChecker(final BaseTrigger trigger, final T stimulus,
            final TargetGroup targetGroup, final Set<Class<? extends ExpectedPrimitive>> allowedExpectedPrimitives,
    ...
    public FilterResult doProcess(final FilterObjectWrapper event) {
        if (event.getEventToFilter() instanceof final MeasurementMade measurementMade) {
            ...
            /*
             * If all the values are collected, then check whether the expected percentage matches,
             * otherwise disregard.
             */
