...
import org.palladiosimulator.elasticity.triggers.expectations.ExpectedPrimitive;
import org.palladiosimulator.elasticity.triggers.expectations.NoExpectation;
import org.palladiosimulator.elasticity.triggers.stimuli.Stimulus;

import com.google.common.base.Preconditions;

/**
 * A checker that should compare the values or simulation state to the expected value. This abstract
 * class provides a utility method {@link #compareToTrigger(double)} to compare the current measured
 * value to the expected value.
 * 
 * @author Julijan Katic
 *
 * @param <T>
 *            The stimulus of the trigger
 */
public abstract class TriggerChecker<T extends Stimulus> implements Filter {
     ...
     * @param stimulusType
     *            The stimulus type.
     * @param allowedExpectedPrimitives
     *            Set of all allowed expected primitives for this trigger.
     */
    TriggerChecker(final BaseTrigger trigger, final Class<T> stimulusType,
        ...
        Preconditions.checkArgument(stimulusType.isAssignableFrom(trigger.getStimulus()
            .getClass()),
                String.format("Trigger must be of type %s, but is %s", stimulusType.getSimpleName(),
                        trigger.getStimulus()
                            .getClass()
                            .getSimpleName()));
    }
    ...
    /**
     * Utility method to compare the defined expected value to the actual {@code value}. Returns a
     * comparator result to be used for further processing (see {@link ComparatorResult} for further
     * description).
     * 
     * @param value
     *            The actual, measured value to compare.
    ...
    protected ComparatorResult compareToTrigger(final double value) {
        if (trigger instanceof SimpleFireOnValue) {
            ...
            } else if (allowedExpectedPrimitives.stream()
                .anyMatch(cls -> cls.isAssignableFrom(trigger.getExpectedValue()
                    .getClass()))) {
