package org.palladiosimulator.analyzer.slingshot.behavior.elasticity.interpreter.entity.adjustor;
 ...
 * which should trigger an adjustment.
 *
...
public class Adjustor implements Filter {
    ...
    public FilterResult doProcess(final FilterObjectWrapper objectWrapper) {
        final double currentSimTime = objectWrapper.getEventToFilter()
            .time();
        // We reached the end, so we can safely say that here the adjustment happens.
        objectWrapper.getState()
            .setLatestAdjustmentAtSimulationTime(objectWrapper.getEventToFilter()
                .time());
        objectWrapper.getState()
            .incrementNumberScales();

        objectWrapper.getState()
            .getTargetGroupState()
            .addEnactedPolicy(currentSimTime, policy);

        final Optional<CooldownConstraint> cooldownConstraint = policy.getPolicyConstraints()
            .stream()
            .filter(obj -> obj instanceof CooldownConstraint)
            .map(obj -> (CooldownConstraint) obj)
            .findAny();

        if (cooldownConstraint.isPresent()) {
            double cooldownTime = cooldownConstraint.get()
                .getCooldownTime();

            if (cooldownTime != 0) {
                if (currentSimTime > objectWrapper.getState()
                    .getCoolDownEnd()) {
                    // cooldown ended
                    objectWrapper.getState()
                        .setNumberOfScalesInCooldown(0);
                    objectWrapper.getState()
                        .setCoolDownEnd(currentSimTime + cooldownTime);
                } else if (currentSimTime == objectWrapper.getState()
                    .getCoolDownEnd()) {
                    objectWrapper.getState()
                        .setCoolDownEnd(currentSimTime + cooldownTime);
                } else {
                    objectWrapper.getState()
                        .incrementNumberOfAdjustmentsInCooldown();
                }
            }
        }
